	...
	initPaymentResp, _, err := apiClient.InstallmentPlanApi.InstallmentPlanInitiate(
		...
		splitit.InitiateInstallmentPlanRequest{
			PlanData: &splitit.PlanData{
				// Capture is initiated by calling StartInstallments
				AutoCapture:          false,
...
func StartInstallments(installmentPlanNumber string) (err error) {
	ctx := context.TODO()

	_, _, err = apiClient.InstallmentPlanApi.InstallmentPlanStartInstallments(
		ctx,
	...
	if err != nil {
		return fmt.Errorf("InstallmentPlanStartInstallments call failed: %w", err)
	}
	return
}
